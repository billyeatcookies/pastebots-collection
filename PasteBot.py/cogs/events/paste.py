...
regex = "(\`{3}[a-z]*\n[\s\S]*?\n\`{3})"
client = pastemyst.Client(key='', is_dev=True)
...
class Paste(commands.Cog):
    ...
    async def on_message(self, msg):
        ...
        ).set_author(name="Pasted!", url=link, icon_url=msg.author.icon_url)
    ...
    async def paste_message(msg):
        ...
        for match in matches:
            ...
            if not language.isspace() or language.strip():
                ...
                pasty.language = pastemyst_language.name | pastemyst.Language.AUTODETECT
